	...
	public :
		...
		void payment(struct src *t){
			...
			if(n > 100 ){
				...
				bufn = table[n][1] ; 
						cout << "you should pay " << bufn << " Toman."; 
						finalmoney += bufn ;
		...
		if(table[bufn][2] == 0 ){
			cout << "FREE" << '\n' ; 
				...
				if((table[bufn][3] >= t->second ) && (table[bufn][4]-(t->msecond) >= 0 )){
					cout << "BUSY" << endl ; 
				}else{
					cout << "PENDING" << endl ;
				}
		...
		{
			for(int i = 0 ; i <= 100 ; i++ ){
				if((table[i][3] >= t->second ) && (table[i][4]-(t->msecond) >= 0 )){
					ServiceTable--; 
					FreeTable++ ; 
				}	
			}

		cout << finalmoney << " " << money-finalmoney << " " << wait << " " <<  eating << " " << ended << " " << FreeTable << " " << ServiceTable << " " 
			<< EatingTable << endl ; 
		}
};
